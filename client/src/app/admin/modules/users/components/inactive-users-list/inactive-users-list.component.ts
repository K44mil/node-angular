...
export class InactiveUsersListComponent implements OnInit {
    ...
    clearQuery() {
        this.clearSelect();
        this.query = `?limit=${this.itemsPerPage}&page=${this.currentPage}&isVerified=0`;
    }
        ...
        this.usersService.activateUser(id)
            ...
            .subscribe(
                res => {
                    ...
                    this.alertService.success('User account has been activated', {
                        autoClose: true
                    });
                    this.loadUsers(this.query);
                },
    ...
    onActionsSelectChange(e) {
        switch (e.target.value) {
            case 'delete-many':
                ...
                break;
            case 'activate-many':
                const btnActivate = document.getElementById('btnActivateManyConfirmationModal');
                btnActivate.click();
    ...
    activateSelectedUsers() {
        if (this.selectedItems.length > 0) {
            this.usersService.activateManyUsers(this.selectedItems)
            .pipe(first())
            .subscribe(
                res => {
                    this.alertService.clear();
